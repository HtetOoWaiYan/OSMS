...
grant update on table "storage"."prefixes" to "service_role";
  ...
  create policy "Give users authenticated access to folder 10abb02_0"
  on "storage"."objects"
  as permissive
  for select
  to authenticated
using (((bucket_id = 'item-images'::text) AND ((storage.foldername(name))[1] = 'private'::text) AND (auth.role() = 'authenticated'::text)));



  create policy "Give users authenticated access to folder 10abb02_1"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check (((bucket_id = 'item-images'::text) AND ((storage.foldername(name))[1] = 'private'::text) AND (auth.role() = 'authenticated'::text)));



  create policy "Give users authenticated access to folder 10abb02_2"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'item-images'::text) AND ((storage.foldername(name))[1] = 'private'::text) AND (auth.role() = 'authenticated'::text)));
  ...
  create policy "Give users authenticated access to folder 10abb02_3"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
