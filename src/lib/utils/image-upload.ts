...
export async function uploadItemImage(
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = supabaseAdmin.storage.from('item-images').getPublicUrl(filePath);

    // If this is set as primary, unset other primary images first
    if (isPrimary) {
      await supabaseAdmin.from('item_images').update({ is_primary: false }).eq('item_id', itemId);
    }
    ...
    if (dbError) {
      ...
      await supabaseAdmin.storage.from('item-images').remove([filePath]);
...
export async function deleteItemImage(imageId: string): Promise<{
  ...
  try {
    ...
    const { error: dbError } = await supabaseAdmin.from('item_images').delete().eq('id', imageId);
...
export async function updateItemImage(
  ...
  try {
    ...
    // Check user permissions
    const projectIdForUpdate = (image.items as { project_id: string }).project_id;
    const { data: userRole } = await supabase
      .from('user_roles')
      .select('role')
      .eq('user_id', user.id)
 ...
 */
export async function deleteAllItemImages(itemId: string): Promise<{
  success: boolean;
  error?: string;
}> {
  try {
