...
export const createItemPriceSchema = z
  .object({
  ...
  .refine(
    (data) => !data.effectiveUntil || data.effectiveFrom || new Date() <= data.effectiveUntil,
    {
...
export const itemFiltersSchema = z.object({
  ...
  sortBy: z.enum(['name', 'created_at', 'stock_quantity', 'price']).default('created_at'),
...
export const skuGenerationSchema = z.object({
  ...
  prefix: z.string().max(10, 'Prefix must be less than 10 characters').optional(),
});
