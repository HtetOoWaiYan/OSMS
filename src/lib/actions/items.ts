...
export async function createItemActionSimple(formData: FormData) {
  try {
    ...
    const initialStockQuantityStr =
      formData.get('initialStockQuantity') || formData.get('stockQuantity');
    const initialMinStockLevelStr =
      formData.get('initialMinStockLevel') || formData.get('minStockLevel');
    ...
    const itemData = createItemSchema.parse({
      ...
      categoryId:
        formData.get('categoryId') === 'no-category' ? null : formData.get('categoryId') || null,
      initialStockQuantity: initialStockQuantityStr
        ? parseInt(initialStockQuantityStr as string, 10)
        : 0,
      initialMinStockLevel: initialMinStockLevelStr
        ? parseInt(initialMinStockLevelStr as string, 10)
        : 0,
...
export async function updateItemActionSimple(formData: FormData) {
  try {
    ...
    const { id, basePrice, sellingPrice, discountPercentage, ...updateData } = itemData;
    ...
    const hasPriceChanges = Object.values(priceFields).some((val) => val !== undefined);
